...
class HealthManager {
    ...
    public func loadMapData(for run: MylesRun) async -> Bool {
        let id = run.id
        guard let locationPoints = await fetchWorkoutLocationData(for: id), !locationPoints.isEmpty else {
...
extension HealthManager {
    ...
    private func fetchWorkouts(type: HKWorkoutActivityType = .running, startDate: Date? = nil, limit: Int = HKObjectQueryNoLimit) async -> [HKWorkout]? {
        ...
        let samples = try? await withCheckedThrowingContinuation { (continuation: CheckedContinuation<[HKSample], Error>) in
            ...
            store.execute(HKSampleQuery(sampleType: .workoutType(), predicate: compoundPredicate, limit: limit, sortDescriptors: [sortDescriptor],
    ...
    private func fetchWorkoutRoutes(for workout: HKWorkout) async -> [HKWorkoutRoute]? {
        ...
        let samples = try? await withCheckedThrowingContinuation { (continuation: CheckedContinuation<[HKSample], Error>) in
            store.execute(HKAnchoredObjectQuery(type: HKSeriesType.workoutRoute(), predicate: byWorkout, anchor: nil, limit: HKObjectQueryNoLimit, resultsHandler: { (_, samples, _, _, error) in
                if let error = error {
    ...
    private func fetchWorkoutLocationData(for id: UUID) async -> [CLLocation]? {
        ...
        let samples = try? await withCheckedThrowingContinuation { (continuation: CheckedContinuation<[HKSample], Error>) in
            store.execute(HKSampleQuery(sampleType: .workoutType(), predicate: workoutPredicate, limit: HKObjectQueryNoLimit, sortDescriptors: nil, resultsHandler: { _, samples, error in
                if let error = error {
    ...
    func calculateMileSplits(startTime: Date, endTime: Date) async -> [TimeInterval] {
        ...
        let samples = try? await withCheckedThrowingContinuation { (continuation: CheckedContinuation<[HKSample], Error>) in
            store.execute(HKSampleQuery(sampleType: distanceType!, predicate: predicate,
                                        limit: HKObjectQueryNoLimit, sortDescriptors: [.init(keyPath: \HKSample.startDate, ascending: true)], resultsHandler: { (_, samples, error) in
                if let error = error {
